...
void KkmParameters::setPayCashMoney(double payCashMoney){
    if(payCashMoney < 0){
        throw std::runtime_error("Сумма оплаты наличными не может быть отрицательной");
    }
...
void KkmParameters::setPayBankCardMoney(double payBankCardMoney){
    if(payBankCardMoney < 0){
        throw std::runtime_error("Сумма оплаты безналичными не может быть отрицательной") ;
    }
