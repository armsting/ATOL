...
{
    std::wstring_convert<std::codecvt_utf8<wchar_t>> convert;
    connection.setSerialPort(convert.from_bytes(ui->comboBox_COMPort->currentText().toStdString())); //Задали текущий COM порт
...
Baudrate MainWindow::convertIntToBaudrate(int Baudrate){
    switch (Baudrate) { //Задаём скорость порта
        case 1200 :
...
void MainWindow::on_pushButton_setCashier_clicked(){
    ...
    if(CashierINN == ""){
        ...
        ui->label_CurrentCashier_Name_INN->setText(codec->toUnicode("Текущий кассир:   ") +
                                                   QString(CashierName.c_str()) + codec->toUnicode(",   ИНН:  ") + QString(CashierINN.c_str()));
    }
